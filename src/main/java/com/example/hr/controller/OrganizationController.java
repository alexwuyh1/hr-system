...
public class OrganizationController {
  ...
  public List<OrganizationNode> positionTree() {
    ...
    return buildPositionTree(depts, positions, null);
  }

  private List<OrganizationNode> buildPositionTree(List<Organization> depts, List<Organization> positions, Long parentId) {
    return depts.stream()
        .filter(d -> (d.getParent() == null && parentId == null) || (d.getParent() != null && d.getParent().getId().equals(parentId)))
        .map(d -> {
          ...
          List<OrganizationNode> positionNodes = positions.stream()
              ...
              .toList();
          node.children = Stream.concat(
              buildPositionTree(depts, positions, d.getId()).stream(),
